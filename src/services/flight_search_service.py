...
from src.models.database import Flight, Result
    ...
    def search_flights(
        ...
        for dep_airport in possible_departures:
            for arr_airport in possible_arrivals:
                for arr_date in possible_arrival_dates:
                    entry = (arr_airport, dep_airport, arr_date)
                    date_search = arr_date.strftime('%Y-%m-%d')
                    flights = self.scraper.get_flights(
                        arr_airport, dep_airport, date_search
                    )
        ...
        for (dep_air, arr_air, dep_dt), dep_flights in departure_flights.items():
            for (
                ...
                ret_arr_air,
                arr_dt,
            ), arr_flights in arrival_flights.items():

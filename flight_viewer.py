...
def main():
    ...
    for filename, df in csv_files.items():
        ...
        columns_to_remove = [col for col in df.columns if 
                           col.endswith('_id') or 
                           'search_date' in col.lower() or
                           col.startswith('dep_id') or
                           col.startswith('arr_id')]
    ...
    original_filenames = {}
    for friendly_name in csv_files.keys():
    ...
    sorted_tab_names = sorted(csv_files.keys(), 
                             key=lambda name: extract_departure_date(original_filenames.get(name, name)))
    ...
    for i, filename in enumerate(sorted_tab_names):
        ...
        with tabs[i]:
            ...
            if 'trip_duration_days' in df.columns and duration_range is not None:
                filtered_df = filtered_df[
                    (filtered_df['trip_duration_days'] >= duration_range[0]) & 
                    (filtered_df['trip_duration_days'] <= duration_range[1])
                ]
            
            if 'dep_companies' in df.columns and selected_companies:
                mask = filtered_df['dep_companies'].apply(
                    lambda x: any(company in str(x) for company in selected_companies) if pd.notna(x) else False
                )
            ...
            with sort_col1:
                sort_column = st.selectbox(
                    ...
                    index=0 if 'total_price' not in filtered_df.columns else filtered_df.columns.tolist().index('total_price'),
                    key=f"sort_col_{i}"
                )
            ...
            if len(display_df) == 0:
                ...
                if total_pages > 1:
                    page = st.selectbox(
                        f"Page (showing {items_per_page} flights per page)",
            ...
            st.download_button(
                label=f"📥 Download filtered data",
                data=csv_data,
